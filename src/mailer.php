 ...
 *
 * Copyright (c) 2021 ~ witchiiwork
 * Witchaphat Muangjansorn <witchiiwork@gmail.com>
 * https://github.com/witchiiwork/f3-mailer/
 *
 * @version 1.0.0
 * @date: 24.03.2021
 */
...
class Mailer {
    /** @var SMTP */
    protected $charset, $message = [], $recipients = [], $smtp;
    ...
    public function initSMTP() {
        ...
        if(!$f3->devoid("mailer.email.reply", $email_reply)) {
            $this->setReply($email_reply);
        }

        if(!$f3->devoid("mailer.email.from", $email_from)) {
            if($f3->devoid("mailer.name.from", $name_from)) {
                $name_from = null;
            }
    ...
    protected function encode($str) {
        ...
        if(!isset($out) || !$out) {
            $out = extension_loaded("mbstring") ? mb_convert_encoding($str,$this->charset, "UTF-8") : utf8_decode($str);
        }
    ...
    protected function encodeHeader($str) {
        if(extension_loaded("iconv")) {
            $out = iconv_mime_encode("Subject", $str, ["input-charset" => "UTF-8", "output-charset" => $this->charset]);
    ...
    public function setHTML($message) {
        ...
        if($f3->exists("mailer.jumplinks", $jumplink) && $jumplink) {
            $tmpl->extend("a", function($node) use($f3, $tmpl) {
                ...
                if(isset($attr["href"])) {
                    if(!$f3->exists("mailer.route_jump", $ping_route)) {
                        $ping_route = "/mailer-jump";
                    }

                    $attr["href"] = $f3->get("SCHEME") . "://" . $f3->get("HOST") . $f3->get("BASE") . $ping_route . "?target=" . urlencode($attr["href"]);
                }
    ...
    public function save($filename) {
        ...
        for($i = 0, $max = count($lines); $i < $max; $i++) {
            if(!$start && preg_match("/^354.*?$/", $lines[$i], $matches)) {
                ...
                continue;
            } elseif(preg_match("/^250.*?$\s^QUIT/m", $lines[$i] . ($i + 1 < $max ? "\n" . $lines[$i + 1] : ""), $matches)) {
                break;
            }
    ...
    static public function ping(\Base $f3, $params) {
        ...
        $img = new \Image();
        $img->load(base64_decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABAQMAAAAl21bKAAAABGdBTUEAALGPC/xhBQAAAANQTFRFAAAAp3o92gAAAAF0Uk5TAEDm2GYAAAAKSURBVAjXY2AAAAACAAHiIbwzAAAAAElFTkSuQmCC"));
        $img->render();
    }
    ...
    static public function initTracking() {
        ...
        if(!$f3->exists("mailer.route_ping", $route_ping)) {
            $route_ping = "/mailer-ping/@hash.png";
        }

        $f3->route("GET " . $route_ping, "\Mailer::ping");

        if(!$f3->exists("mailer.route_jump", $route_jump)) {
            $route_jump = "/mailer-jump";
        }

        $f3->route("GET " . $route_jump, "\Mailer::jump");
    }
}
